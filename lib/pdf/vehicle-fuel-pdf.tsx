...
import type { VehicleFuelHistory } from "../types/fuel";

interface VehicleFuelPDFProps {
  fuelHistory: VehicleFuelHistory;
  dateRange: { from: string; to: string };
}

export function VehicleFuelPDF({
  fuelHistory,
  dateRange,
}: VehicleFuelPDFProps) {
  const { vehicle, records, anomaliesDetected, anomalyRecords } = fuelHistory;
  ...
  // Split records into pages (20 records per page)
        ...
        return (
          <Page key={pageIndex} style={pdfStyles.page}>
            ...
            {pageIndex === 0 && (
              <>
                <View>
                  <Text style={pdfStyles.header}>VEHICLE FUEL REPORT</Text>
                ...
                {pageRecords.map((record, index) => {
                  ...
                  return (
                    <View key={record.id || index} style={pdfStyles.tableRow}>
                      ...
                      >
                        {record.driverId.slice(0, 18) || "N/A"}
                      </Text>
                      <Text
                      ...
                      <Text
                        style={[pdfStyles.tableCellSmall, { width: "12%" }]}
                      >
                        {calculateEfficiency(record.distanceKm, record.liters)}
                      </Text>
                      <Text
                        style={[pdfStyles.tableCellSmall, { width: "15%" }]}
                      >
            ...
            <View style={pdfStyles.footer} fixed>
              <Text>
                Gas System - Vehicle Fuel Report ({vehicle.plate}) | Page{" "}
